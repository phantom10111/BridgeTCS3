...
TEST(ScorerTest, SimpleClubsWithDouble)
{
	...
	model::DealResult dealResult( model::Contract(1, model::Denomination::CLUBS, 2 ,0), 7);
...
TEST(ScorerTest, SimpleSlamBonud)
{
	...
	model::DealResult dealResult( model::Contract(7, model::Denomination::NO_TRUMP, 4 ,0), 13);
	ASSERT_TRUE(scorer.update(dealResult));
	model::GameResult gameResult = scorer.getResult();
...
TEST(ScorerTest, FullRuberGame)
{
	...
	model::DealResult dealResultOne( model::Contract(2, model::Denomination::NO_TRUMP, 1 ,0), 9);
	...
	ASSERT_EQ(0, gameResult.under[1]);
	model::DealResult dealResultTwo( model::Contract(4, model::Denomination::HEARTS, 1 ,1), 10);
	...
	ASSERT_EQ(0, gameResult.under[0]);
	ASSERT_EQ(0, gameResult.under[1]);
	model::DealResult dealResultThree( model::Contract(5, model::Denomination::CLUBS, 1 ,1), 9);
	...
	ASSERT_EQ(0, gameResult.under[0]);
	ASSERT_EQ(0, gameResult.under[1]);
	model::DealResult dealResultFour( model::Contract(4, model::Denomination::SPADES, 2 ,0), 11);
	...
	ASSERT_EQ(0, gameResult.under[0]);
	ASSERT_EQ(0, gameResult.under[1]);
	model::DealResult dealResultFive( model::Contract(3, model::Denomination::CLUBS, 1 ,0), 10);
	...
	ASSERT_EQ(0, gameResult.under[1]);
	model::DealResult dealResultSix( model::Contract(6, model::Denomination::DIAMONDS, 1 ,1), 12);
